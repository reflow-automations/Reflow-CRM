...
import { ICE_BUCKET_CONFIG, ICE_STATUS_CONFIG, type ICEBucket, type ICEStatus, type ICEKind } from '@/lib/constants'
...
import { ICEItemDialog } from './ICEItemDialog'
...
export function ICEPage() {
  ...
  const [dialogKind, setDialogKind] = useState<ICEKind>('oneoff')
  ...
  // Alleen 'oneoff' doet mee in de ICE-scoring/sortering. Doorlopende verplichtingen
  // en wachtende items staan op een apart bord (/doorlopend).
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-text-muted">Eenmalige taken, geprioriteerd met Impact, Importance, Time en Difficulty</p>
        </div>
      ...
      {showInfo && (
        <div className="rounded-xl border border-border bg-surface p-5">
          ...
          <p className="mt-3 text-xs text-text-dim">
            Dit bord toont alleen <span className="text-sky-400 font-medium">eenmalige</span> taken. Doorlopende verplichtingen en wachtende items staan op het <span className="text-violet-400 font-medium">Doorlopend</span>-bord.
          </p>
        </div>
      )}
      ...
      <p className="text-xs text-text-dim">
        {filteredItems.length} van {oneoffItems.length} eenmalige taken
      </p>
